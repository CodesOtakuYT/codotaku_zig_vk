...
pub const GraphicsContext = struct {
    ...
    pub fn init(allocator: Allocator, app_name: [*:0]const u8, window: *c.SDL_Window) !GraphicsContext {
        ...
        const getProcAddr: vk.PfnGetInstanceProcAddr = @ptrCast(c.SDL_Vulkan_GetVkGetInstanceProcAddr().?);
        ...
        const glfw_exts = c.SDL_Vulkan_GetInstanceExtensions(&glfw_exts_count).?;
...
fn createSurface(instance: Instance, window: *c.SDL_Window) !vk.SurfaceKHR {
    ...
    if (!c.SDL_Vulkan_CreateSurface(window, @ptrFromInt(@intFromEnum(instance.handle)), null, &surface)) {
...
fn initializeCandidate(instance: Instance, candidate: DeviceCandidate) !vk.Device {
    ...
    var vulkan_features13 = vk.PhysicalDeviceVulkan13Features{
        .dynamic_rendering = .true,
        .synchronization_2 = .true,
    };

    var vulkan_features14 = vk.PhysicalDeviceVulkan14Features{
        .push_descriptor = .true,
        .p_next = &vulkan_features13,
    };

    return try instance.createDevice(candidate.pdev, &.{
        .p_next = &vulkan_features14,
